(function() {
    ...
    ui.innerHTML = `
        ...
        <div id="input-wrapper">
            ...
            <button id="announcement-btn" class="action-btn" title="Ogłoszenie">📢</button>
    ...
    async function sendMessage(text, isAnnouncement = false) {
        if (!text.trim()) return;
        const finalMsg = isAnnouncement ? `[OGŁOSZENIE] ${text}` : text;
        try {
            await fetch(chatURL, {
                method: 'POST',
                body: JSON.stringify({ 
    ...
    async function fetchMessages() {
        ...
        try {
            ...
            Object.keys(data).forEach(id => {
                ...
                const isAnn = m.msg.startsWith("[OGŁOSZENIE]");
            ...
            if (lastAnnouncement) {
                ...
                pinnedBox.innerHTML = `📌 <b>${lastAnnouncement.nick}:</b> ${lastAnnouncement.msg.replace("[OGŁOSZENIE]", "")}`;
            } else {
    ...
    announceBtn.onclick = () => {
        if(input.value.trim()) sendMessage(input.value, true);
    };
