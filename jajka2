(function() {
    ...
    let memory = {a1:[], a2:[], e3:[]}, lastTarget = "", currentData = null;

    const ui = document.createElement('div');
    ui.style.cssText = `position:fixed;top:${savedPos.top};left:${savedPos.left};z-index:10001;background:rgba(10,10,10,0.95);color:#f0f0f0;padding:7px;border:2px solid #e67e22;border-radius:8px;font-family:sans-serif;font-size:11px;width:115px;user-select:none;box-shadow:0 4px 15px rgba(0,0,0,0.8);`;
    ...
    function report() {
        let txt = `--- RAPORT BŁĘDU MALOWANIA ---\n`;
        txt += `CEL: ${lastTarget}\n\n`;
        txt += `PAMIĘĆ:\nD1: ${JSON.stringify(memory.a1)}\nD3: ${JSON.stringify(memory.a2)}\nD2: ${JSON.stringify(memory.e3)}\n\n`;
        txt += `MAPA (GÓRA -> DÓŁ):\n`;
        currentData.all.forEach(i => {
            txt += `[Pos: ${i.top}px, X: ${i.x}] -> ${i.s} (${i.file})\n`;
        });
        
        console.clear();
        console.log("%cSKOPIUJ PONIŻSZY TEKST:", "color: #f1c40f; font-weight: bold; font-size: 14px;");
        console.log(txt);
        alert("Raport wygenerowany w konsoli (F12). Jest teraz w formie jednego bloku tekstu do skopiowania.");
    }

    function scan() {
        let d = {target:[], e1:[], a1:[], e2:[], a2:[], e3:[], all:[]};
        document.querySelectorAll('.animator-clip').forEach(clip => {
            ...
            let item = {s:val, x, top, file:f};
            d.all.push(item);
            if (top===0 && colorMap[f]) d.target.push(item);
            else if (top===160 && colorMap[f]) d.e1.push(item);
            else if (top===192 && arrowMap[f]) d.a1.push({dir:arrowMap[f],x});
            else if (top===224 && colorMap[f]) d.e2.push(item);
            else if (top===256 && arrowMap[f]) d.a2.push({dir:arrowMap[f],x});
            else if (top===288 && colorMap[f]) d.e3.push({s:colorMap[f],x});
        });
        Object.keys(d).forEach(k => { if(Array.isArray(d[k])) d[k].sort((a,b)=>a.x-b.x); });
    ...
    function update() {
        ...
        let html = `<div style="display:flex;justify-content:space-between;margin-bottom:5px;border-bottom:1px solid #444;"><b style="color:#f1c40f;font-size:10px;">MALOWANIE</b><button id="res-btn" style="background:#95a5a6;color:white;border:none;border-radius:2px;font-size:9px;padding:0 3px;cursor:pointer;">R</button></div>`;
        if (sol) {
            ...
            html += `<div style="color:${full?'#e74c3c':'#95a5a6'};text-align:center;font-size:10px;padding:4px;">${full?'BRAK OPCJI':'KLIKAJ...'}</div>`;
        }
        html += `<button id="err-btn" style="width:100%;margin-top:5px;background:none;color:#555;border:1px dashed #333;font-size:8px;cursor:pointer;">RAPORT ANALIZY</button>`;
        ui.innerHTML = html;
        ui.querySelector('#res-btn').onclick = () => { memory={a1:[],a2:[],e3:[]}; };
        ui.querySelector('#err-btn').onclick = report;
    }
