(function() {
    ...
    // TYLKO TE GEMY BĘDĄ LICZONE (Ścisła lista)
    const allowedGems = {
        "Atak Plus": 4, 
    ...
    // Ulepszone czyszczenie nazwy - usuwa wszystko poza czystym tekstem i cyframi
    function getCleanedName(el) {
        let text = el.innerText || "";
        // Usuwa ikony, emotki i znaki specjalne, zostawia tylko tekst i spacje
        return text.replace(/[^a-zA-Z0-9 ąćęłńóśźżĄĆĘŁŃÓŚŹŻ]/g, '').replace(/\s+/g, ' ').trim();
    }
    ...
    function countGems() {
        ...
        gems.forEach(gem => {
            ...
            const name = getCleanedName(nameEl);

            // SPRAWDZENIE: Czy nazwa jest na naszej białej liście?
            if (allowedGems.hasOwnProperty(name)) {
                const stars = gem.querySelectorAll('.star.full').length;
                const limit = allowedGems[name];
                
                const isPlus2 = name.endsWith("2");
                const category = isPlus2 ? "plus2" : "plus";
    ...
    function renderList(obj, color) {
        ...
        if (keys.length === 0) return ''; // Nie wypisuje nic, jeśli nie ma gemów z listy
    ...
    function updateUI(data) {
        ...
        if (isMinimized) {
            ...
            // Budowanie HTML tylko z sekcji, które mają dane
            let html = '';
            
            // Sekcja Zwykłe Plusy
            if (Object.keys(data.plus.ready).length > 0) {
                html += `<div style="color: #55ff55; font-weight: bold; font-size: 10px; margin-bottom: 3px;">✅ PLUS (GOTOWE)</div>
                         <div style="margin-bottom: 12px; padding-left: 5px; border-left: 2px solid #55ff55;">${renderList(data.plus.ready, '#55ff55')}</div>`;
            }
            if (Object.keys(data.plus.todo).length > 0) {
                html += `<div style="color: #ff5555; font-weight: bold; font-size: 10px; margin-bottom: 3px;">❌ PLUS (DO WBIJANIA)</div>
                         <div style="margin-bottom: 12px; padding-left: 5px; border-left: 2px solid #ff5555;">${renderList(data.plus.todo, '#ff5555')}</div>`;
            }

            // Sekcja Plus 2
            if (Object.keys(data.plus2.ready).length > 0 || Object.keys(data.plus2.todo).length > 0) {
                html += `<div style="color: #55aaff; font-weight: bold; font-size: 10px; margin-bottom: 3px; border-top: 1px solid #333; padding-top: 5px;">💎 PLUS 2 (MAX = 3★)</div>
                         <div style="margin-bottom: 5px; padding-left: 5px; border-left: 2px solid #55aaff;">`;
                if (Object.keys(data.plus2.ready).length > 0) {
                    html += `<div style="color: #55ff55; font-size: 9px; opacity: 0.8;">Gotowe:</div>${renderList(data.plus2.ready, '#55ff55')}`;
                }
                if (Object.keys(data.plus2.todo).length > 0) {
                    html += `<div style="color: #ff5555; font-size: 9px; opacity: 0.8; margin-top: 3px;">Do wbijania:</div>${renderList(data.plus2.todo, '#ff5555')}`;
                }
                html += `</div>`;
            }

            content.innerHTML = html || '<div style="color:#777; text-align:center;">Brak gemów z listy</div>';
        }
    }
    ...
    function createPanel() {
        ...
        panel.appendChild(header); panel.appendChild(content); document.body.appendChild(panel);
        document.getElementById('gem-js-min-btn').onclick = (e) => {
            isMinimized = !isMinimized; e.target.innerText = isMinimized ? "□" : "_"; countGems();
        };
        ...
        header.onmousedown = (e) => {
            ...
            document.onmousemove = (e) => {
                ...
                let nTop = panel.offsetTop - p2; let nLeft = panel.offsetLeft - p1;
